...
│   │   └── communication\_ltd\_vulnerable.db
...
SELECT \* FROM users WHERE username = 'admin' --' AND password\_hash = '...'
...
&#x20; INSERT INTO customers (full\_name, email, phone, sector\_id, package\_id)
...
SESSION\_SECRET=change\_this\_session\_secret

HMAC\_SECRET=change\_this\_hmac\_secret
...
| secure-version     | `secure-version/database/communication\_ltd\_secure.db`         |

| vulnerable-version | `vulnerable-version/database/communication\_ltd\_vulnerable.db` |
...
sqlite3 secure-version/database/communication\_ltd\_secure.db < database-scripts/schema.sql

sqlite3 secure-version/database/communication\_ltd\_secure.db < database-scripts/seed.sql
...
sqlite3 vulnerable-version/database/communication\_ltd\_vulnerable.db < database-scripts/schema.sql

sqlite3 vulnerable-version/database/communication\_ltd\_vulnerable.db < database-scripts/seed.sql
...
sqlite3 secure-version/database/communication\_ltd\_secure.db < database-scripts/reset.sql
...
sqlite3 vulnerable-version/database/communication\_ltd\_vulnerable.db < database-scripts/reset.sql
...
| `password\_history`      | Stores previous password hashes to prevent password reuse                                 |

| `password\_reset\_tokens` | Stores reset tokens, expiry time and usage state                                          |

| `internet\_packages`     | Stores available internet packages                                                        |

| `sectors`               | Stores customer sectors                                                                   |

| `subscription\_requests` | Stores package subscription requests                                                      |
...
| `password\_hash`   | HMAC-SHA256 password hash                    |

| `salt`            | Random salt                                  |

| `role`            | User role: admin, employee, viewer, customer |

| `package\_id`      | Assigned internet package                    |

| `failed\_attempts` | Failed login counter                         |

| `locked\_until`    | Temporary account lock timestamp             |
...
sqlite3 secure-version/database/communication\_ltd\_secure.db
...
sqlite3 vulnerable-version/database/communication\_ltd\_vulnerable.db
