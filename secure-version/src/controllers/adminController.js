...
// Promise wrappers make the review transaction easier to follow.
...
async function upsertCustomerRecord(requestRow) {
  ...
  // Approval upgrades the user into the customer list. Re-approving or
  // approving a later package request updates the existing record in place.
  if (existingCustomer) {
    await runQuery(
      `UPDATE customers
...
async function reviewRequest(req, res, nextStatus) {
  ...
  try {
    ...
    // Request status, user role/package changes and customer sync all happen in
    // one transaction so admins do not leave the system in a half-updated state.
