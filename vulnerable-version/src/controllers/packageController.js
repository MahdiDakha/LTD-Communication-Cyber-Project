...
// Promise-based sqlite helpers keep these flows aligned with the rest of the
// codebase and make the package workflow easier to scan.
...
export async function showPackagesPage(req, res) {
  try {
    ...
    // One page serves several roles: browsing for everyone, active package info
    // for customers, and request tracking for viewers.
    if (req.session.user) {
...
export async function requestPackage(req, res) {
  ...
  try {
    ...
    // Viewers can submit only one outstanding request so admin review remains
    // the single point that advances package assignment.
    if (pendingRequest) {
      return res.redirect(
        "/packages?error=" +
          encodeURIComponent(
