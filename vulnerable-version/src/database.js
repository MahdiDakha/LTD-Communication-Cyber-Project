...
const dbPath = path.join(databaseDir, "communication_ltd_vulnerable.db");
...
const db = new sqlite3.Database(dbPath, (err) => {
  if (err) {
    console.log("Vulnerable database connection error:", err.message);
  } else {
    console.log(`Vulnerable database connected successfully: ${dbPath}`);
  }
});

db.serialize(() => {
  // Keep the vulnerable demo schema aligned with the secure build while
  // ensuring its SQLi and XSS demo data stays isolated in its own database.
  ...
  db.run(
    ...
    (alterErr) => {
      if (
        ...
        console.log("Vulnerable users role migration error:", alterErr.message);
      }
    },
  );
  ...
  db.run(
    ...
    (alterErr) => {
      if (
        ...
        console.log(
          "Vulnerable users package_id migration error:",
          alterErr.message,
        );
      }
    },
  ...
  db.run(
    ...
    (updateErr) => {
      if (updateErr) {
        console.log(
          "Vulnerable users role normalization error:",
          updateErr.message,
        );
      }
    },
